...
.endif
    ...
    jc L_dg_moveuint64sthen1
      ...
      addq %rdx, %rdi
      addq %rdx, %rsi
      subq $8, %rdi /* post decrement with std need this */
      subq $8, %rsi /* post decrement with std need this */
      std
...
_dg_setbit:
    ...
    mov %rdi, %rax // mov rax, rcx
    bts %rsi, %rax // bts rax, rdx
    ret
...
_dg_twototheu:
    ...
    xor %rax, %rax
    inc %rax
    shl %cl, %rax // shl rax, cl
    ret

// rdi = ; rcx = u
// rsi = ; rdx = which set bit to find from lo to hi (1 = first set  bit)
.globl _dg_scanforuthsetbit
_dg_scanforuthsetbit:

    mov %rdi, %rax // mov rax, rcx
    mov $0x40,%rcx // mov rcx, 40h

    or %rsi, %rsi // or rdx, rdx      ; /* testing for 0 */
    jnz _dg_scanforbitthen1
      mov $0x40, %rax // mov rax, 40h   ; not found
      ret
    _dg_scanforbitthen1:

    _dg_scanforbitbegin:
      shr $1, %rax // shr rax, 1
      jnc _dg_scanforbitthen2
        dec %rsi // dec rdx
        jz _dg_scanforbitthen3        
      _dg_scanforbitthen2:
      dec %rcx
    jnz _dg_scanforbitbegin

    _dg_scanforbitthen3:
    mov $0x40, %rax // mov rax, 40h
    sub %rcx, %rax // sub rax, rcx
    ret


.globl _dg_getulowestsetbits
_dg_getulowestsetbits:

    mov %rdi, %rax // mov rax, rcx
    mov %rax, %r8 // mov r8, rax
    mov $0x40, %rcx // mov rcx, 40h

    or %rsi, %rsi // or rdx, rdx      ; /* testing for 0 */
    jnz _dg_getulowestsetbitsthen1
      mov %rsi, %rax // mov rax, rdx   ; /* if getting no bits, return 0 */
      ret
    _dg_getulowestsetbitsthen1:

    _dg_getulowestsetbitsbegin:
      shr $1, %rax // shr rax, 1
      jnc _dg_getulowestsetbitsthen2
        dec %rsi // dec rdx
        jz _dg_getulowestsetbitsthen3        
      _dg_getulowestsetbitsthen2:
      dec %rcx
    jnz _dg_getulowestsetbitsbegin
    ...
    inc %rax // inc rax
    cmp $0x40, %rcx // cmp rcx, 40h
    jc _dg_getulowestsetbitsthen4
      dec %rax
      dec %rax
      and %r8, %rax // and rax, r8
      ret
    _dg_getulowestsetbitsthen4:
    shl %cl, %rax // shl rax, cl
    dec %rax
    and %r8, %rax // and rax, r8
    ret
...
// rdi = ; rcx = uvalue
// rsi = ; rdx = ucount = number of bits to get
.globl _dg_getulowestbits
_dg_getulowestbits:
    
    mov %rdi, %rdx // xchg rcx, rdx
    mov %rsi, %rcx

    // ; might not need to do this if overshifting gives 0
    cmp $0x40, %rcx // cmp rcx, 40h
    jc _dg_getulowestbitsthen2
      mov %rdi, %rax // mov rax, rdx
      ret
    _dg_getulowestbitsthen2:
    ...
    dec %rax
    and %rdi, %rax // and rax, rdx
    ret


// rdi = ; rcx = ucount = number of set bits in mask
.globl _dg_getulowestbitsmask
_dg_getulowestbitsmask:
 
    mov %rdi, %rcx
    // ; might not need to do this if overshifting gives 0
    cmp $0x40, %rcx // cmp rcx, 40h
    jc _dg_getulowestbitsmaskthen2
      xor %rax, %rax
      dec %rax
      ret
    _dg_getulowestbitsmaskthen2:
...
// does:
//   dest - src -> flags
.globl _dg_compareforconditionsub
_dg_compareforconditionsub:
